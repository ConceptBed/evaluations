...
if __name__=="__main__":
    ...
    domains = ["attribute", "action", "counting", "relation"]

    for dm in domains:
        dataset[dm] = {}
        metrics = {
            "accuracy": Accuracy(),
            "delta_new": DeltaNew(domain=dm),
        }
        for k, v in metrics.items():
            dataset[dm][k] = []

        for model in models:
            y_true, logits = load_data(model.format(dm))
            y_true_org, logits_org = load_data(args.org_name.format(dm))

            for k, _ in metrics.items():
                metrics[k].update(logits, y_true, y_true_org, logits_org)
                dataset[dm][k].append(metrics[k].get_score())

        df = pd.DataFrame(dataset[dm])
        df.index = models
        df.to_csv(os.path.join(store_path, f"compositions_{dm}_all.csv"))


    new_df = {k: [] for k, v in metrics.items()}
    for metric in list(metrics.keys()):
        tmp = []
        for dm in domains:
            tmp.append(dataset[dm][metric])
        new_df[metric] = np.mean(tmp, axis=0)

    df = pd.DataFrame(new_df)
    df.index = models
    df.to_csv(os.path.join(store_path, f"compositions_{args.gen_name}_overall.csv"))
    tex = df.to_latex()
    with open(os.path.join(store_path, f"compositions_{args.gen_name}_overall.tex"), "w") as h:
        h.write(tex)
